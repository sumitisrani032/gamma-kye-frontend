...
interface NavItem {
  ...
  action?: string;
  minScope?: string;
  /** If set, only show if user has this role */
  requireRole?: string;
}
...
export function TenantSidebar() {
  ...
  // Settings visible if user has management-level access (not just read for dropdowns)
  const canSeeSettings =
    canAccessModule("tenant_settings") ||
    canWithScope("company", "read", "global") ||       // Org structure (Admin, HR Dir)
    canWithScope("role", "read", "global") ||           // Roles (Admin)
    canWithScope("audit_log", "read", "global") ||      // Audit logs (Admin, HR Dir)
    canWithScope("policy_document", "read", "global") ||// Policy docs (HR Mgr+)
    canWithScope("document_requirement", "read", "global") || // Doc reqs (HR Dir+)
    canWithScope("workflow", "create", "global") ||     // Workflow CRUD (HR Dir+)
    canWithScope("employee_document", "verify", "global"); // Doc verify (HR Mgr+)
    ...
    <aside className="flex h-full w-64 flex-col bg-surface border-r border-border">
      ...
      <div className="border-t border-border px-4 py-4">
        <div className="flex items-center gap-3">
          <Link href="/my-profile" className="flex items-center gap-3 flex-1 min-w-0 hover:opacity-80 transition-opacity">
